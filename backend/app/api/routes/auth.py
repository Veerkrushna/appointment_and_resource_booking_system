...
def _auth_response(user: User) -> AuthResponse:
    return AuthResponse(access_token=create_access_token(user.id, user.role), user=CustomerResponse.model_validate(user))
...
def register_customer(payload: CustomerRegister, db: Annotated[Session, Depends(get_db)]):
